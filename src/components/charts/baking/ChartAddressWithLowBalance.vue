<template>
  <CommonChart
    ...
    label="Number of address (balance <1 xtz)"
    title="Number of address (balance <1 xtz)"
...
<script>
  ...
  export default {
    name: 'ChartAddressWithLowBalance',
    ...
    methods: {
      async fetch(options) {
        ...
        try {
          const response = await this.$api.getLowBalanceTotalAgg(options);
