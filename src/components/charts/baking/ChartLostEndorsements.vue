<template>
  <CommonChart
    ...
    label="Lost endorsements"
    title="Lost endorsements"
  ...
  export default {
    ...
    data() {
      return {
        ...
        data: null,
        datePoints: null,
        timeOptions: [timeType.lastWeek, timeType.lastMonth],
      };
    },
    mounted() {
    ...
    methods: {
      async fetch(options) {
        ...
        try {
          const response = await this.$api.getLostEndorsementsAgg(options);
