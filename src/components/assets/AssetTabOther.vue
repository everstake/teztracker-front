  ...
  <div class="list asset-other">
    <div class="d-flex justify-content-between mb-2">
      <LimitSelect
        :limit="limit"
        @onLimitChange="
          (limit) => $emit('onLimitChange', { name: 'otherOperations', limit })
        "
        :loading="loading"
      />
    </div>
    ...
    <PaginationSelect
      ...
      :current-page="page"
      :loading="loading"
      @onPageChange="(page) => $emit('onPageChange', { name: 'otherOperations', page })"
    />
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if (itemsNotFetched) {
        this.$emit('onReload', { name: 'otherOperations', page, limit });
      }
    },
  };
</script>
